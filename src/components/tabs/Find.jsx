...
export default function FindTab({ search, setSearch, playerGames, tournaments, publishedDraws = [], drawPairings = [], onH2H }) {
  ...
  if (selectedDraw) {
    ...
    // Preview overlay — full-screen iframe of the print sheet
    if (showPreview) {
      ...
      return (
        <div style={{ position: "fixed", inset: 0, zIndex: 9998, background: "rgba(0,0,0,0.6)", display: "flex", flexDirection: "column" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "8px", padding: "10px 14px", background: "#1a1a1a", flexShrink: 0 }}>
            ...
            <div style={{ flex: 1, fontFamily: F_UI, fontSize: "13px", fontWeight: "700", color: "#fff" }}>{draw.tournament_name} — Sheet Preview</div>
          ...
          <iframe src={blobUrl} style={{ flex: 1, border: "none", background: "#fff" }}
            onLoad={() => URL.revokeObjectURL(blobUrl)} title="Tie Sheet Preview" />
        </div>
      );
    }
